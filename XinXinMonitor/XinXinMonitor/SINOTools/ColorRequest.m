//
//  ColorRequest.m
//  ChilliCircle
//
//  Created by 孙瑞 on 15/10/22.
//  Copyright © 2015年 SinoGlobal. All rights reserved.
//

#import "ColorRequest.h"

@implementation ColorRequest
+ (UIColor *)TabBarTextColor {
    return [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1];
}

+ (UIColor *)TabBarBackgroundColor {
    return [UIColor colorWithRed:248/255.0 green:248/255.0 blue:248/255.0 alpha:1];
}

+ (UIColor *)TabBarLineColor {
    return [UIColor colorWithRed:242/255.0 green:242/255.0 blue:242/255.0 alpha:1];
}

+ (UIColor *) MainBlueColor {
    return [UIColor colorWithRed:30/255.0 green:162/255.0 blue:239/255.0 alpha:1];
}

+ (UIColor *) TextColor55 {
    return [UIColor colorWithRed:55/255.0 green:55/255.0 blue:55/255.0 alpha:1];
}

+ (UIColor *) TextColor153 {
    return [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1];
}

+ (UIColor *) BackGroundColor {
    return [UIColor colorWithRed:239/255.0 green:239/255.0 blue:244/255.0 alpha:1];
}

+ (UIColor *) grayColor {
    return [UIColor colorWithRed:200/255.0 green:200/255.0 blue:200/255.0 alpha:1];
}

+ (UIColor *) DividingLineColor {
    return [UIColor colorWithRed:217/255.0 green:217/255.0 blue:217/255.0 alpha:1];
}

+ (UIColor *) ShowListNowColor {
    return [UIColor colorWithRed:250/255.0 green:173/255.0 blue:16/255.0 alpha:1];
}

@end
